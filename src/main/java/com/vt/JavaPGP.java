...
public final class JavaPGP {
  ...
  /**
   * The message is created like so: - Generates a random KeyPair - Encrypt the message with the
   * private key from the generated key pair - Encrypt the generated public key with given public
   * key
   *
  ...
  public static byte[] encrypt(byte[] message, PublicKey key) throws GeneralSecurityException {
    ...
    ByteBuffer buffer =
        ByteBuffer.allocate((encryptedPublicKey.length + encryptedMessage.length) + 4);
  ...
  /**
   * The message is decrypted like so: - Read the encrypted public key - Decrypt the public key with
   * the private key - Read the encrypted message - Use the decrypted public key to decrypt the
   * encrypted message
   *
