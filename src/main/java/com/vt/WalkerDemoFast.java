...
public class WalkerDemoFast {
    ...
    ClassData(Lookup lookup, Class<?> klass) throws Throwable {
      for (Method m : klass.getMethods()) {
        ...
        if (Collection.class.isAssignableFrom(m.getReturnType())) {
          ...
          if (param instanceof Class<?> && Node.class.isAssignableFrom((Class<?>) param))
  ...
  public void walk(Node node, Consumer<Node> visitor) throws Throwable {
    ...
    ClassData data =
        class_data.computeIfAbsent(
            klass,
            k -> {
              try {
                return new ClassData(lookup, k);
              } catch (Throwable e) {
    ...
    for (NodesGetter getter : data.compound_getters) {
      ...
      for (Node child : children) walk(child, visitor);
    }
  }
}
